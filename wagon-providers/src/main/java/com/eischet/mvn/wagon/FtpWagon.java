 ...
 */
@Component(role = org.apache.maven.wagon.Wagon.class, hint = "eischet-ftps")
public class FtpWagon extends StreamWagon {
    ...
    @Configuration(name = "passiveMode", value = "true")
    protected boolean passiveMode = true;

    @Configuration(name = "controlEncoding", value = "ISO-8859-1")
    ...
    protected ConnectionInfo getConnectionInfoForTransfer() throws TransferFailedException {
        try {
    ...
    protected ConnectionInfo getConnectionInfo() throws AuthenticationException {
        final Repository myRepository = getRepository();
        AuthenticationInfo authInfo = Objects.requireNonNull(getAuthenticationInfo(), "authenticationInfo cannot be null");
        ...
        if (password == null) {
            throw new AuthenticationException("Password not specified for repository " + myRepository.getId());
        }

        final RepositoryPermissions permissions = repository.getPermissions();


        int port = WagonConstants.UNKNOWN_PORT;
        String host = myRepository.getHost();
        String basedir = myRepository.getBasedir();
        if (uploadUrl != null && !uploadUrl.isEmpty()) {
            final URI uploadUri = URI.create(uploadUrl);
            host = uploadUri.getHost();
            if (uploadUri.getPort() > 0) {
                port = uploadUri.getPort();
            }
            basedir = uploadUri.getPath();
            if (basedir.isEmpty()) {
                basedir = "/";
            } else if (basedir.charAt(0) != '/') {
                basedir = "/" + basedir;
            }
            if (uploadUri.getQuery() != null) {
                log.warn("Ignoring query part of uploadUrl: " + uploadUri.getQuery());
            }
        }

        return new ConnectionInfo(host, port, basedir, username, password, passiveMode, controlEncoding, permissions);
    }
    ...
    protected void openConnectionInternal() throws ConnectionException, AuthenticationException {
        ...
        try {
            if (connectionInfo.getPort() != WagonConstants.UNKNOWN_PORT) {
                ftp.connect(connectionInfo.getHost(), connectionInfo.getPort());
            } else {
                ftp.connect(connectionInfo.getHost());
            }
        ...
        try {
            if (!ftp.login(connectionInfo.getUsername(), connectionInfo.getPassword())) {
                throw new AuthenticationException("Cannot login to remote system");
            }

            performAfterLoginSteps(ftp);
    ...
    public void fillOutputData(OutputData outputData) throws TransferFailedException {
        final ConnectionInfo connectionInfo = getConnectionInfoForTransfer();
        OutputStream os;
        Resource resource = outputData.getResource();
        RepositoryPermissions permissions = connectionInfo.getPermissions();
        try {
            if (!ftp.changeWorkingDirectory(connectionInfo.getBasedir())) {
                throw new TransferFailedException("Required directory: '" + connectionInfo.getBasedir() + "' " + "is missing");
            }
            ...
            if (os == null) {
                String msg = "Cannot transfer resource:  '" + resource + "'. Output stream is null. FTP Server response: " + ftp.getReplyString();
    ...
    private void ftpChangeDirectory(Resource resource) throws IOException, TransferFailedException, ResourceDoesNotExistException {
        final ConnectionInfo connectionInfo = getConnectionInfoForTransfer();
        if (!ftp.changeWorkingDirectory(connectionInfo.getBasedir())) {
            throw new ResourceDoesNotExistException("Required directory: '" + connectionInfo.getBasedir() + "' " + "is missing");
        }
    ...
    public boolean resourceExists(String resourceName) throws TransferFailedException, AuthorizationException {
        ...
        try {
            ...
            return ((status == FTPReply.FILE_STATUS) || (status == FTPReply.DIRECTORY_STATUS) || (status == FTPReply.FILE_STATUS_OK) // not in the RFC but used by some FTP servers
    ...
    public void putDirectory(File sourceDirectory, String destinationDirectory) throws TransferFailedException, ResourceDoesNotExistException, AuthorizationException {
        ...
        try {
            if (!ftp.changeWorkingDirectory(connectionInfo.getBasedir())) {
                RepositoryPermissions permissions = connectionInfo.getPermissions();
                if (!makeFtpDirectoryRecursive(connectionInfo.getBasedir(), permissions)) {
                    throw new TransferFailedException("Required directory: '" + connectionInfo.getBasedir() + "' " + "could not get created");
                }

                // try it again sam ...
                if (!ftp.changeWorkingDirectory(connectionInfo.getBasedir())) {
                    throw new TransferFailedException("Required directory: '" + connectionInfo.getBasedir() + "' " + "is missing and could not get created");
                }
            }
        } catch (IOException e) {
            throw new TransferFailedException("Cannot change to root path " + connectionInfo.getBasedir(), e);
        }

        fireTransferDebug("Recursively uploading directory " + sourceDirectory.getAbsolutePath() + " as " + destinationDirectory);
    ...
    private void ftpRecursivePut(File sourceFile, String fileName) throws TransferFailedException {
        ...
        if (sourceFile.isDirectory()) {
            if (!fileName.equals(".")) {
                try {
                    ...
                    if (!ftp.changeWorkingDirectory(fileName)) {
                        // first, try to create it
                        if (makeFtpDirectoryRecursive(fileName, connectionInfo.getPermissions())) {
                            if (!ftp.changeWorkingDirectory(fileName)) {
                    ...
                    throw new TransferFailedException("IOException caught while processing path at " + sourceFile.getAbsolutePath(), e);
                }
            }
            ...
            try {
                ...
                throw new TransferFailedException("IOException caught while attempting to step up to parent directory" + " after successfully processing " + sourceFile.getAbsolutePath(), e);
            }
        } else {
            ...
            try {
                ...
                if (ftp.storeFile(fileName, sourceFileStream)) {
                    final RepositoryPermissions permissions = connectionInfo.getPermissions();
                    if (permissions != null) {
                    ...
                    String msg = "Cannot transfer resource:  '" + sourceFile.getAbsolutePath() + "' FTP Server response: " + ftp.getReplyString();
                    throw new TransferFailedException(msg);
                }
                ...
                throw new TransferFailedException("IOException caught while attempting to upload " + sourceFile.getAbsolutePath(), e);
            } finally {
    ...
    private boolean makeFtpDirectoryRecursive(String fileName, RepositoryPermissions permissions) throws IOException {
        if (fileName == null || fileName.isBlank() || fileName.replace('/', ' ').isBlank()) { // if a string is '/', '//' or similar
            return false;
        }
        ...
        if (slashPos >= 0 && slashPos < (fileName.length() - 1))  {
            // not only a slash at the end, like in 'newDir/'
            if (oldPwd == null) {
                oldPwd = ftp.printWorkingDirectory();
            }
